...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? nft-asset-class token-id sender recipient)))
...
(define-read-only (get-token-uri (token-id uint))
  (ok (some (unwrap-panic (map-get? cids token-id)))))

;; #[allow(unchecked_data)]
(define-public (claim (uris (list 25 (string-ascii 256))))
  (mint-many uris))

(define-private (mint-many (uris (list 25 (string-ascii 256))))
  (let 
    (
...
(define-private (mint-many-iter (uri (string-ascii 256)) (next-id uint))
  (begin
